...
@section('title')
<title>Astroaquila | Edit Product Sub Category</title>
@endsection
...
<div class="content-page">
    ...
    <div class="content">
        <div class="container">
            ...
            <div class="row">
                <div class="col-sm-12">
                    <h4 class="pull-left page-title">Edit Product Sub Category</h4>
                    <ol class="breadcrumb pull-right">
                        <li class="active"><a href="{{route('admin.product.category.manage')}}"><i
                    ...
                    <div>
                        ...
                        <div class="panel panel-default panel-fill">
                            <div class="panel-heading">
                                <h3 class="panel-title">Edit Product Sub Category</h3>
                            </div>
                            <div class="panel-body rm02 rm04">
                                <form role="form" action="{{route('admin.product.sub-category.update')}}" method="POST" id="productCategoryFrom" enctype="multipart/form-data">
                                    @csrf
                                    ...
                                    <select class="form-control rm06 basic-select" name="category" id="category">
                                        ...
                                        <option value="{{@$value->id}}" @if($productCategory->parent_id == @$value->id)selected @endif>{{@$value->name}}</option>
                                        @endforeach
                                    </select>
                                    ...
                                    <div class="form-group">
                                        ...
                                        <input type="text" placeholder="Sub Category name" id="subcategory" class="form-control" name="subcategory_name"  value="{{$productCategory->name}}">
                                        <div id="err_sub_category"></div>
                                    </div>
                                  ...
                                  <div class="form-group" style="position: relative;">
                                       <a class="del_image del-custom-class" data-id="{{@$productCategory->id}}" @if(@$productCategory->image=='') style="display:none" @endif > <i class="fa fa-times" aria-hidden="true"></i> </a>
                                          <div class="uplodimgfilimg ">
                                            <em><img src="{{ URL::to('storage/app/public/product_category_image')}}/{{@$productCategory->image}}" alt="" id="img2"></em>
                                          </div>
                                  ...
                                  <div class="clearfix"></div>
                                    <div class="form-group rm03">
                                        ...
                                        <textarea style="height: 80px" id="description" class="form-control" name="description" placeholder="Category description" >{{@$productCategory->description}}</textarea>
                                    </div>
                                    ...
                                    <div class="form-group rm03">
                                        ...
                                        <input type="text" placeholder="Meta title" id="meta_title" class="form-control new-form" name="meta_title" value="{{@$productCategory->meta_title}}">
                                    </div>
                                    <div class="form-group rm03">
                                        <label for="meta_description">Meta description</label>
                                        <textarea style="height: 80px" id="meta_description" class="form-control" name="meta_description" placeholder="Meta description" >{{@$productCategory->meta_description}}</textarea>
                                    </div>


                                    <div class="clearfix"></div>
                                    <div class="col-lg-12"> <button class="btn btn-primary waves-effect waves-light w-md" type="submit">Update</button></div>
                                </form>
...
<script type="text/javascript">
  $('.del_image').on('click',function(e){
    ...
    $('#profile_picture').val('');
    $.ajax({
      url:'{{route('admin.product.category.delete.image')}}',
      type: "POST",
      data:{
         id:id,
        _token: '{{ csrf_token() }}',
...
<script>
    $(document).ready(function(){
        ...
        errorPlacement: function(error, element) {
            ...
            if (element.attr("name") == "subcategory_name") {
               $("#err_sub_category").append(error);
            }
        ...
        submitHandler: function(form){
                ...
                $.ajax({
                      url:"{{route('admin.product.sub-category-check')}}",
                      method:"GET",
                      data:{'category':category,'subcategory':subcategory,'id':id},
                      success: function(res) {
                      console.log(res);  
                      if (res=="found") {
