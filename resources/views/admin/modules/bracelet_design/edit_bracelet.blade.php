...
@section('title')
<title>Astroaquila | Edit Bracelet Design</title>
@endsection
...
<div class="content-page">
    ...
    <div class="content">
        <div class="container">
            ...
            <div class="row">
                <div class="col-sm-12">
                    <h4 class="pull-left page-title"> Edit Bracelet Design</h4>
                    <ol class="breadcrumb pull-right">
                        <li class="active"><a href="{{route('admin.manage.bracelet.design')}}"><i class="fa fa-arrow-left" aria-hidden="true"></i> Back</a></li>
                    </ol>
                </div>
            </div>
            <div class="row">
                    ...
                    <div>
                        ...
                        <div class="panel panel-default panel-fill">
                            <div class="panel-heading">
                                <h3 class="panel-title">Edit Bracelet Design</h3>
                            </div>
                            <div class="panel-body rm02 rm04">
                                <form role="form"  method="POST" action="{{route('admin.manage.bracelet.design.update')}}" id="design_form" enctype="multipart/form-data">
                                    @csrf
                                    ...
                                    <div class="form-group">
                                                 ...
                                                 <select class="form-control rm06 basic-select" name="metal" id="metal">
                                                  ...
                                                  <option value="G" @if(@$data->metal_type=='G') selected @endif>Gold</option>
                                    ...
                                    <div class="form-group">
                                        <label for="category_name">Design Name</label>
                                        <input type="text" placeholder="Name" id="name" class="form-control" name="name" required value="{{@$data->design_name}}">
                                        <div id="err_name"></div>
                                        <div id="err_duplicate" style="display: none;color: red;">Design Name Already Exits</div>
                                    </div>
                            ...
                            <div class="form-group" style="position: relative;">
                                <a class="del_image del-custom-class" data-id="{{@$data->id}}" @if(@$data->design_picture=='') style="display:none " @endif > <i class="fa fa-times" aria-hidden="true"></i></a>  
                                <div class="uplodimgfilimg ">
                                            <em><img src="{{ URL::to('storage/app/public/bracelet_design')}}/{{@$data->design_picture}}" alt="" id="img2"></em>
                                 </div>
                               </div>
            ...
            if (files.length > 0) {
             ...
             <script type="text/javascript">
                ...
                errorPlacement: function(error, element) {
                 console.log("Error placement called");
                     if (element.attr("name") == "name") {
                      ...
                      else if (element.attr("name") == "metal") {
                         
                          $("#error_metal").append(error);
                      }
                       else if (element.attr("name") == "price_inr") {
                         
                          $("#err_inr").append(error);
                      }
                      else if (element.attr("name") == "price_usd") {
                         
                          $("#err_usd").append(error);
                      }
                      // else if (element.attr("name") == "profile_picture") {
              ...
              if ($('#profile_picture').val()=='' &&  $('#image_url').val()=='') {
                alert('Please upload bracelet image');
                return false;
              }
              $.ajax({
                      url:"{{route('admin.manage.bracelet.design.check')}}",
                      method:"GET",
                      data:{'name':name,'id':id,'metal':metal},
                      success: function(res) {
                      console.log(res);  
                      if (res=="found") {
