...
@section('title')
<title>Astroaquila | Edit Horoscope Title</title>
@endsection
...
<div class="content-page">
    ...
    <div class="content">
        <div class="container">
            ...
            <div class="row">
                <div class="col-sm-12">
                    <h4 class="pull-left page-title">Edit Horoscope Title</h4>
                    <ol class="breadcrumb pull-right">
                        <li class="active"><a href="{{route('admin.manage.horoscope.title')}}"><i class="fa fa-arrow-left" aria-hidden="true"></i> Back</a></li>
                    </ol>
                </div>
            </div>
            <div class="row">
                    ...
                    <div>
                        ...
                        <div class="panel panel-default panel-fill">
                            <div class="panel-heading">
                                <h3 class="panel-title">Edit Horoscope Title</h3>
                            </div>
                            <div class="panel-body rm02 rm04">
                                <form role="form" action="{{route('admin.manage.horoscope.title.update')}}" method="POST" id="pujacat" enctype="multipart/form-data">
                                    @csrf
                                    <input type="hidden" name="id" value="{{@$data->id}}">
                                    <div class="form-group">
                                        <label for="category_name">Title</label>
                                        <input type="text" placeholder="Title" id="title" class="form-control" name="title" required value="{{@$data->title}}">
                                        <div id="err_category"></div>
                                    </div>
                                    <div class="clearfix"></div>
                                    <div class="form-group">
                                        <label for="Email">Title Image</label>
                                        <div class="uplodimgfil">
                                            ...
                                            <label for="icon">Upload Title Image<img src="{{ URL::to('public/admin/assets/images/clickhe.png')}}" alt=""></label>
                                        </div>
                                       <div id="err_icon"></div>
                                  </div>
                                  ...
                                  <div class="form-group" style="position: relative;">
                                       <a class="del_image del-custom-class" data-id="{{@$data->id}}" @if(@$data->image=='') style="display:none " @endif > <i class="fa fa-times" aria-hidden="true"></i> </a>
                                          <div class="uplodimgfilimg ">
                                            <em><img src="{{ URL::to('storage/app/public/horoscope_title')}}/{{@$data->image}}" alt="" id="img2"></em>
                                          </div>
...
<script type="text/javascript">
  $('.del_image').on('click',function(e){
    ...
    $('#profile_picture').val('');
    $.ajax({
      url:'{{route('admin.hororscope.title.delete.image')}}',
      type: "POST",
      data:{
         id:id,
        _token: '{{ csrf_token() }}',
       ...
       $("#pujacat").validate({
            rules: {
                title: {
                   required:true,
                   remote: {
                          url:  '{{route('admin.manage.horoscope.title.check')}}',
                          type: "POST",
                          data: {
                            title: function() {
                              return $( "#title").val() ;
