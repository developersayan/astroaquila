				...
				<div class="row">
					<div class="col-12">
						<div class="inner-headings">
							<h3>Horoscope Sub Categories</h3>
			...
			<div class="row">
				<div class="col-md-12">
					<ul class="cayegoies_uls">
						...
						<?php 
									$array = [];
									$gettitle =  DB::table('horoscope')->where('sub_category_id',@$value->id)->where('status','A')->pluck('title_id')->toArray();   
								    $common  = array_unique($gettitle);
								    $countTitle = count($common);
						?>
						<a href="{{route('horoscope.title.get',['id'=>@$value->id])}}">
						<li>
							
							<div class="categ-lists">
								
								<a href="{{route('horoscope.title.get',['id'=>@ $value->id])}}" class="lists_icons">
							    @if(@$value->image=='')		
								...
								<div class="lists_li">								
									<div class="text-left">
										<a href="{{route('horoscope.title.get',['id'=>@ $value->id])}}">{{@$value->name}}</a>
										
									<p>{{@$countTitle}} Title</p>
									</div>
									
									<a href="{{route('horoscope.title.get',['id'=>@ $value->id])}}" class="nest"><img src="{{asset('public/frontend/images/submitarw1.png')}}" alt=""></a>
