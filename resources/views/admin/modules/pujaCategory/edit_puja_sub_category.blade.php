...
@section('title')
<title>Astroaquila | Edit  Sub Category</title>
@endsection
...
<div class="content-page">
    ...
    <div class="content">
        <div class="container">
            ...
            <div class="row">
                <div class="col-sm-12">
                    <h4 class="pull-left page-title">Edit Sub Category</h4>
                    <ol class="breadcrumb pull-right">
                        <li class="active"><a href="{{route('admin.manage.puja-category')}}"><i class="fa fa-arrow-left" aria-hidden="true"></i> Back</a></li>
                    </ol>
                </div>
            </div>
            <div class="row">
                    ...
                    <div>
                        ...
                        <div class="panel panel-default panel-fill">
                            ...
                            <div class="panel-body rm02 rm04">
                                <form role="form" action="{{route('admin.manage.puja.subcategory.update')}}" method="POST" id="pujacat" enctype="multipart/form-data">
                                    @csrf
                                    ...
                                    <div class="form-group">
                                        ...
                                        <input type="text" placeholder="Category Name" id="name" class="form-control" name="name" required value="{{@$data->name}}">
                                  ...
                                  <div class="form-group" style="position: relative;">
                                       <a class="del_image del-custom-class" data-id="{{@$data->id}}" @if(@$data->image=='') style="display:none " @endif > <i class="fa fa-times" aria-hidden="true"></i></a>
                                          <div class="uplodimgfilimg ">
                                            <em><img src="{{ URL::to('storage/app/public/puja_cat_image')}}/{{@$data->image}}" alt="" id="img2"></em>
                                          </div>
...
<script type="text/javascript">
  $('.del_image').on('click',function(e){
    ...
    $('#profile_picture').val('');
    $.ajax({
      url:'{{route('admin.manage.puja.category.delete.image')}}',
      type: "POST",
      data:{
         id:id,
        _token: '{{ csrf_token() }}',
       ...
       $("#pujacat").validate({
        ...
        messages: {
            category: {
             ...
             name:{
                required:'Please enter name of subcategory',
             },  
          },
          submitHandler: function(form){
            ...
            // alert(title);
                 $.ajax({
                         url:"{{route('admin.manage.puja.subcategory.check-name')}}",
                         method:"GET",
                         data:{'category':category,'name':name,'id':id},
                         success: function(res) {
                         console.log(res);  
                         // return false;
                          if (res=="found") {
                         alert('Category already exits in this parent category');
                         return false;
                         }else{
                          form.submit();                  
                        }
