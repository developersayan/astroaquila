		...
		<div class="container">
			<div class="row">
				<div class="col-lg-3 bor-r">
					...
					<div class="search-filter">
                        <form action="{{route('horoscope.search')}}" method="POST" id="filter">
                            @csrf
                                ...
                                <div class="panel panel-default">
                                    <div class="panel-heading" role="tab" id="headingnew">
                                        <h3 class="panel-title">
                                            <a data-toggle="collapse"  href="#collapsenew" aria-expanded="false" aria-controls="collapsenew">Find Horoscope</a>
                                        </h3>
                                    </div>
						 ...
						 <span style="float:right;"><a href="{{route('horoscope.search')}}">Reset All</a></span></div>
						...
						<div class="panel panel-default">
							<div class="diiferent-sec @if(@$request['category']) selected-single-value @endif">
								<select class="login-type log-select basic-select" name="subcat" class="subcat"  id="subcat_filter">
						...
						@endif
						@endif
						@if(@$hscopetitles->isNotEmpty())
						<div class="panel panel-default">
							<div class="diiferent-sec @if(@$request['category']) selected-single-value @endif">
								<select class="login-type log-select basic-select" name="title_id" class="title_id"  id="title_filter">
									<option value="">Select Title</option>
									@foreach (@$hscopetitles as $key=>$title)
									<option value="{{$title->id}}" {{ @$request['title_id']==$title->id?'selected':'' }}>{{$title->title}}</option>
									@endforeach
								</select>
							</div>
                        </div>	
            ...
            @if(@$expertise->isNotEmpty())
                     ...
                     <div class="diiferent-sec  rashi_select" >
                                    <select class="chosen-select form-control  " multiple data-placeholder="Select Expertise" name="expertise[]" id="expertise_filter">
