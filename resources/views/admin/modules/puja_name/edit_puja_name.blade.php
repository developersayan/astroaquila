...
@section('title')
<title>Astroaquila | Edit Puja Name</title>
@endsection
...
<div class="content-page">
    ...
    <div class="content">
        <div class="container">
            ...
            <div class="row">
                <div class="col-sm-12">
                    <h4 class="pull-left page-title">Edit Puja Name</h4>
                    <ol class="breadcrumb pull-right">
                        <li class="active"><a href="{{route('admin.manage.puja.name')}}"><i class="fa fa-arrow-left" aria-hidden="true"></i> Back</a></li>
                    </ol>
                </div>
            </div>
            <div class="row">
                    ...
                    <div>
                        ...
                        <div class="panel panel-default panel-fill">
                            <div class="panel-heading">
                                <h3 class="panel-title">Edit Puja Name</h3>
                            </div>
                            <div class="panel-body rm02 rm04">
                                <form role="form" action="{{route('admin.manage.puja.name.update')}}" method="POST" id="cert_name" enctype="multipart/form-data">
                                    @csrf
                                    <input type="hidden" name="id" value="{{@$data->id}}">
                                    <div class="form-group">
                                        <label for="category_name">Puja Name</label>
                                        <input type="text" placeholder="Puja Name" id="name" class="form-control" name="name" required value="{{@$data->name}}">
                                  ...
                                  <div class="form-group" style="position: relative;">
                                       <a class="del_image del-custom-class" data-id="{{@$data->id}}" @if(@$data->image=='') style="display:none " @endif > <i class="fa fa-times" aria-hidden="true"></i> </a>
                                          <div class="uplodimgfilimg ">
                                            <em><img src="{{ URL::to('storage/app/public/puja_name_image')}}/{{@$data->image}}" alt="" id="img2"></em>
                                          </div>
...
<script type="text/javascript">
  $('.del_image').on('click',function(e){
    ...
    $('#profile_picture').val('');
    $.ajax({
      url:'{{route('admin.manage.puja.name.delete.image')}}',
      type: "POST",
      data:{
         id:id,
        _token: '{{ csrf_token() }}',
       ...
       $("#cert_name").validate({
            rules: {
                name: {
                   required:true,
                   remote: {
                          url:  '{{route('admin.manage.puja.name.check')}}',
                          type: "POST",
                          data: {
                            name: function() {
                              return $( "#name").val() ;
