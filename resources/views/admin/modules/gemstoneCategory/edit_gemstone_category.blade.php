...
@section('title')
<title>Astroaquila | Edit Gemstone Category</title>
@endsection
            ...
            <div class="row">
                <div class="col-sm-12">
                    ...
                    <ol class="breadcrumb pull-right">
                        <li class="active"><a href="{{route('admin.gemstone.category.manage')}}"><i
                    ...
                    <div>
                        ...
                        <div class="panel panel-default panel-fill">
                            <div class="panel-heading">
                                <h3 class="panel-title">Edit Gemstone Category</h3>
                            </div>
                            <div class="panel-body rm02 rm04">
                                <form role="form" action="{{route('admin.gemstone.category.edit.save',['id'=>@$GemstoneCategory->id])}}" method="POST" id="gemstoneCategoryFrom" enctype="multipart/form-data">
                                    @csrf
                                    <div class="form-group">
                                        <label for="category_name">Category name</label>
                                        <input type="text" placeholder="Category name" id="category_name" class="form-control" name="category_name"  value="{{$GemstoneCategory->category_name}}">
                                  ...
                                  <div class="form-group" style="position: relative;">
                                       <a class="del_image del-custom-class" data-id="{{@$GemstoneCategory->id}}" @if(@$GemstoneCategory->image=='') style="display:none " @endif > <i class="fa fa-times" aria-hidden="true"></i> </a>
                                          <div class="uplodimgfilimg ">
                                            <em><img src="{{ URL::to('storage/app/public/gemstone_category')}}/{{@$GemstoneCategory->image}}" alt="" id="img2"></em>
                                          </div>
                                  </div>
...
<script type="text/javascript">
  $('.del_image').on('click',function(e){
    ...
    $('#profile_picture').val('');
    $.ajax({
      url:'{{route('admin.gemstone.category.image.delete')}}',
      type: "POST",
      data:{
         id:id,
        _token: '{{ csrf_token() }}',
...
<script>
    $(document).ready(function(){
        $("#gemstoneCategoryFrom").validate({
            rules: {
            category_name: {
               required:true, 
               remote:{
                 url: '{{route('admin.gemstone.checkname')}}',
                          type: "POST",
                          data: {
                            category_name: function() {
                              return $( "#category_name").val() ;
                            },
                            id:'{{@$GemstoneCategory->id}}',
                            _token: '{{ csrf_token() }}',
                          }
               },
           },
