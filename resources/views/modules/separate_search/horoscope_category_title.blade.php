			...
			<div class="row">
				<div class="col-md-12">
					<ul class="cayegoies_uls">
						...
						<?php 
									$array = [];
									$horoscope =  DB::table('horoscope')->where('title_id',@$value->id)->where('category_id',@$category->id)->where('status','A')->pluck('id')->toArray();   
									$count = count($horoscope);
						?>
						<a href="{{route('horoscope.separate.search',['id'=>@$value->id,'cat'=>@$category->id])}}">
						<li>
							
							<div class="categ-lists">
								
								<a href="{{route('horoscope.separate.search',['id'=>@$value->id,'cat'=>@$category->id])}}" class="lists_icons">
							    @if(@$value->image=='')		
								...
								<div class="lists_li">								
									<div class="text-left">
										<a href="{{route('horoscope.separate.search',['id'=>@$value->id,'cat'=>@$category->id])}}">{{@$value->title}}</a>
										
									<p>{{@$count}} Horoscopes</p>
									</div>
									
									<a href="{{route('horoscope.separate.search',['id'=>@$value->id,'cat'=>@$category->id])}}" class="nest"><img src="{{asset('public/frontend/images/submitarw1.png')}}" alt=""></a>
